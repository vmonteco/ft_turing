...
A `Makefile` is provided pour faciliter la compilation et la gestion du projet :

- `make` ou `make all` : compile le projet et génère l'exécutable `ft_turing`.
- `make clean` : supprime les fichiers générés lors de la compilation.
- `make re` : nettoie puis recompile le projet (équivalent à `make clean` puis `make`).

Exemple :

```sh
make
./ft_turing <machine.json> <input>
```
