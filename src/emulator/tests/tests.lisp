...
(def-suite hardware-tests :in emulator-tests)
...
(let ((*hw-blank* #\.))
  ...
  ;; (test init-hardware-tests
  ;; 		(let ((hw (init-hardware "")))
  ;; 		  (is (equal-char (head hw) *blank*)))
  ;; 		(let ((hw (init-hardware "foo")))
  ;; 		  (is (equal-char (head hw) #\f))))

  ;; (test move-right-tests
  ;; 		;; "empty" memory
  ;; 		(let ((hw (make-instance 'hardware :head *blank*)
